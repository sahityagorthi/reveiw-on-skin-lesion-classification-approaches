...
def extract_features(image,mask=None):    
    ...
    for theta in thetas:
        kern = cv2.getGaborKernel((31,31), 3.25, theta, 9.0, 1.0, 1, ktype=cv2.CV_32F)
        kern /= 1.5*kern.sum()
        gabor_filters.append(kern) 
    for kern in gabor_filters:
        filter_img      = cv2.filter2D(masked_lesion_GL, cv2.CV_8UC3, kern)
        gaborGLCM       = greycomatrix(filter_img, [1], [0, np.pi/4, np.pi/2, 3*np.pi/4], levels=2**8)
    ...
    features = np.concatenate((features, GLCM_RGB, GLCM_HSV, GLCM_LAB, LBP_CGLF, entropyplus_RGB, entropyplus_HSV, gabor_energy),axis=0)
